...
const filtered = (products, filterBy) => {
  ...
  if (filterBy.category !== "") {
    filtered = filtered.filter((pro) => pro.categoryName === filterBy.category);
  }
  if (filterBy.price && filterBy.price.length === 2) {
    filtered = filtered.filter((product) => {
      const productPrice = product.price;
      return (
        productPrice >= filterBy.price[0] && productPrice <= filterBy.price[1]
      );
    });
  }
  return filtered;
...
export const productReducer = (state, action) => {
  switch (action.type) {
    ...
    typeof value === "string" && value.toLowerCase().includes(action.payload.toLowerCase())
  )
      );
      return {
        ...state,
    ...
    case "SET_RATE":
      return {
        ...state,
        filterBy: { ...state.filterBy, rate: action.payload },
      };
    case "SET_LOADING":
